"""
BNCI2014_001 EEGNetECA_SCA —— 5折交叉验证（0train上CV，best模型分别在1test上评测；汇总均值±标准差）
...
# ✅ 改动 1：导入 SCA 版本模型
from revisemodel.ecascamodel import EEGNetECA_SCA
...
# ✅ 改动 2：修改保存路径
SAVE_DIR = r"E:\wfr\code\EEG\result_eegnetecasca_cv5"
...
def train_one_fold(fold_id, train_idx, val_idx):
    ...
    # ✅ 改动 3：使用 EEGNetECA_SCA 模型
    model = EEGNetECA_SCA(
        n_chans=n_chans, n_outputs=n_classes,
        input_window_samples=n_times, eca_k=5
    ).to(device)

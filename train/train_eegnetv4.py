"""
BNCI2014_001 EEGNetv4 —— 5折交叉验证（0train上CV，best模型分别在1test上评测；汇总均值±标准差）
...
SAVE_DIR = r"E:\wfr\code\EEG\result_eegnetv4_cv5"
...
def train_one_fold(fold_id, train_idx, val_idx):
    ...
    model = EEGNetv4(n_chans=n_chans, n_outputs=n_classes, input_window_samples=n_times).to(device)

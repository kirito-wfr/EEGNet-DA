"""
BNCI2014_001 EEGNet_SCA —— 5折交叉验证（0train上CV，best模型分别在1test上评测；汇总均值±标准差）
...
from revisemodel.scamodel import EEGNet_SCA
from torch.optim.lr_scheduler import StepLR

# ====================== 基本配置 ======================
device = "cuda" if torch.cuda.is_available() else "cpu"
SAVE_DIR = r"E:\wfr\code\EEG\result_eegnetsca_cv5"
...
def train_one_fold(fold_id, train_idx, val_idx):
    ...
    # ✅ 使用 SCA-only 模型
    model = EEGNet_SCA(n_chans=n_chans, n_outputs=n_classes, input_window_samples=n_times).to(device)
